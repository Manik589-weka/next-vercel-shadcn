...
export interface BlogSection5Author {
  name: string;
  role: string;
  avatar: string;
}

export interface BlogSection5Post {
  id: string | number;
  title: string;
  date: string;
  category: string;
  description: string;
  image: string;
  author: BlogSection5Author;
}

export interface BlogSection5Props {
  /** Blog posts to display. Defaults to placeholder content. */
  posts?: BlogSection5Post[];
  /** Section heading (e.g. "Learn what's new"). */
  heading?: string;
  /** Tagline above heading (e.g. "Blog Section"). */
  tagline?: string;
  /** Optional short description below heading. */
  description?: string;
  className?: string;
}

const DEFAULT_BLOG_POSTS: BlogSection5Post[] = [
  {
    id: 1,
...
export function BlogSection5({
  ...
  tagline = "Blog Section",
  description = "Add a concise value statement that captures your reader's interest and previews your content value.",
  className,
}: BlogSection5Props = {}) {
  return (
    <section
      className={`section-padding-y ${className ?? ""}`}
